...
class DecomposableAttention:
    ...
    def test_on_generator(self, test_generator,steps):
        results = self.model.evaluate_generator(test_generator,steps)
        print(results)
        
        predictions = self.model.predict_generator(test_generator,steps=steps)
        print(predictions.shape)
        print(predictions)
        
        # Save history
        hist_df = pd.DataFrame(predictions)
        hist_df.to_csv('predictions.csv', index=None)
    ...
    def train_on_generator(self, train_generator, valid_generator, n_train, 
        ...
        self.model.save('model_trained_base.h5')
...
if __name__ == '__main__':
    ...
    #model.test_on_generator(sd.batch_generator(indices_set='valid', batch_size=args.batch_size, noise=0),steps=args.batch_size)
