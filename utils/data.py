...
class Data:
    def __init__(self,
        ...
        self.valid_idx = None
        self.test_idx = None
    def setVectors(self,nlp_vectors):
        self.nlp = spacy.load('en_core_web_sm')
        self.nlp.wv = nlp_vectors

        # Check if list of oov vectors exists
        # If so, load, if not, create
        oov_path ='data/glove.840B.300d.snli_pruned.oov.txt'
        self.nlp.oov = np.loadtxt(oov_path)
    def load_nlp(self):
    ...
    def calc_words(self, s):
        return get_words_rep(
                s, 
                self.nlp, 
                normed=self.normed,
                pos_to_remove=self.pos_to_remove,
    ...
    def get_sent_words(self, sent_hash):
        s,h = sent_hash
        MM = self.calc_words(s)
        return MM
    ...
    def batch_generator(self, indices_set='train', batch_size=128,
        ...
        while True:
            ...
            if single_pass:
                break
    def data_test(self):
        #Obtenemos los indices del TEST de los ejemplos
        jdx = self.test_idx.copy()
        j = jdx
        X1, X2 = self.get_features_for_indices(j)
        t = self.targets[j]
        return [X1, X2], t
